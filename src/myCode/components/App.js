...
import Footer from "./Footer";
import Timer from "./Timer";
import { questions as questionsData } from "./questionsData"; // Importing the questions data
  ...
  switch (action.type) {
    case "dataReceived":
      return {
        ...
        questions: action.payload,
        status: "Ready",
        secondsRemaining: action.payload.length * SECs_PER_Question, //30 sec for each question
      };
    case "dataFaild":
      return { ...state, status: "Error" };
    case "start":
      return { ...state, status: "Active" };
    case "newAnswer":
      const question = state.questions[state.index]; //select current question
      return {
        ...state,
        answer: action.payload,
        points:
...
export default function App() {
  ...
  useEffect(() => {
    // Directly dispatching the data received action with the local data
    try {
